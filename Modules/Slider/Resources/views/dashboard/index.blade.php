...
@section('title', __('slider::dashboard.slider.routes.index'))
@section('content')
    ...
    <div class="page-content">
        <div class="page-bar">
            <ul class="page-breadcrumb">
                ...
                <li>
                    <a href="#">{{__('slider::dashboard.slider.routes.index')}}</a>
                </li>
            </ul>
        </div>
                ...
                <div class="portlet light bordered">
                    ...
                    <div class="table-toolbar">
                        <div class="row">
                            <div class="col-md-6">
                                <div class="btn-group">
                                    <a href="{{ url(route('dashboard.slider.create')) }}" class="btn sbold green">
                        ...
                        <table class="table table-striped table-bordered table-hover" id="dataTable">
                            <thead>
                                <tr>
                                    ...
                                    <th>#</th>
                                    <th>{{__('slider::dashboard.slider.datatable.image')}}</th>
                                    <th>{{__('slider::dashboard.slider.datatable.status')}}</th>
                                    <th>{{__('slider::dashboard.slider.datatable.start_at')}}</th>
                                    <th>{{__('slider::dashboard.slider.datatable.end_at')}}</th>
                                    <th>{{__('slider::dashboard.slider.datatable.created_at')}}</th>
                                    <th>{{__('slider::dashboard.slider.datatable.options')}}</th>
                                </tr>
                            </thead>
                        </table>
                    </div>
                    <div class="row">
                        <div class="form-group">
                            <button type="submit" id="deleteChecked" class="btn red btn-sm" onclick="deleteAllChecked('{{ url(route('dashboard.slider.deletes')) }}')">
  ...
  <script>
   function tableGenerate(data='') {
      ...
      $('#dataTable').DataTable({
          ...
          ajax : {
              url   : "{{ url(route('dashboard.slider.datatable')) }}",
              type  : "GET",
              data  : {
                  req : data,
              },
          ...
          columnDefs: [
      			...
      			},
            {
              targets: -1,
              width: '13%',
              title: '{{__('slider::dashboard.slider.datatable.options')}}',
              className: 'dt-center',
              orderable: false,
              render: function(data, type, full, meta) {

                // Edit
      					var editUrl = '{{ route("dashboard.slider.edit", ":id") }}';
      					editUrl = editUrl.replace(':id', data);

      					// Delete
      					var deleteUrl = '{{ route("dashboard.slider.destroy", ":id") }}';
